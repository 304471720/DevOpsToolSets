...
do
    ...
    elif [[ "developer1;developer2;developer3;developer4;developer5;" =~ "${kv[1]}" ]]; then
       echo ${kv[0]}" "${kv[1]} >> "$3".tmp
    fi
done < "$3"
...
getnum $1 $2
ret=$?
cat Statis.txt  | awk '{s[$2] += $1}END{ for(i in s){  print s[i], i } }'  | awk -v time="$ret" '{if($1>=1500 ){ print 1500-($1%125)" "$2 ;}else if($1<=375){print $1+time" "$2;} else print $1" "$2;}' | sort -rn > result.txt
cp result.txt Statis.txt
