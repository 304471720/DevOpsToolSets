...
def sendSms(tel,msgcont):
    if( msgcont != ''):
    	url = '短信通知接口/xxxx.do?name=test&pwd=test&dst='+tel+'&msg='+quote(msgcont)
...
def select():
    while(1):
    	try:
    		conn=MySQLdb.connect(host='192.168.5.242',user='用户名',passwd='密码',db='数据库名',port=3331,charset="utf8")
    		cur=conn.cursor()
		sqlstr = raw_input('>>')
